...
const AUSENCIA = {
VA: { etiqueta: "Vacaciones", icono: " 🌴", color: "#10B981" },
EN: { etiqueta: "Entrenamiento", icono: " 📚", color: "#A78BFA" },
};

const MESES = ["Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"];
constante DOW_S = ["L", "M", "X", "J", "V", "S", "D"];
const TURNO_DEF = {
M: {color: "#F59E0B",etiqueta: "Mañana" },
N: { color: "#818CF8", etiqueta: "Noche" },
D: { color: "#64748B", etiqueta: "Descanso" }
};

// ─── UTILS TEMPORALES ────────────────────────────── ───────────────────────────
const dim = (y, m) => nueva Fecha(y, m + 1, 0).getDate();
...
función autoAssign(ops, targetYear, off) {
const hSC = {}, nSC = {}, pares = {};
...
const [off, setOff] = usePersisted("sc_cycle_offset_ v4", -11);
...
const canEdit = isAdmin || session?.rol === "editor";
...
<button onClick={() => setThemeMode(themeMode === 'dark' ? 'light' : 'dark')} style={{ background: 'none', borde: 'none', fontSize: 18, cursor: 'pointer' }}>{themeMode === 'dark' ? ' ☀️' : ' 🌙'}</button>
...
<button onClick={() => mes === 0 ? (setMonth(11), setAY(activeYear-1)) : setMonth(month-1)} style={{ background: t.card, color: t.text, border: `1px solid ${t.border}`, padding: '8px 15px', borderRadius: 6, cursor: 'pointer' }}>‹</button>
...
{(isExporting ? MENTHS : [MONTHS[month]]).map((mName, mIdx) => {
...
<input id="adP" type="password" placeholder="Pass" style={{ padding: 8, background: t.bg , border: `1px solid ${t.border}`, color: t.texto, ancho: '45%', borderRadius: 4 }} />
...
{ops.map(o => <option key={ o.id } valor={ o.id }>{ o.name }</option> )}
</select>
</div>
